...
def is_valid_single_interval_format(s):
...
def iou_glue_reward(completions, solution, **kwargs): # Modified reward function name and arguments
    ...
    def compute_iou(list_a, list_b):
        # 打印输入信息
        print(f"\nInput list_a: {list_a}")
        print(f"Input list_b: {list_b}")
        
        # 定义无效区间标记
        invalid_interval = [-1.0, -1.0]
        
        # 处理单个区间输入的情况
        if not isinstance(list_a[0], (list, tuple)):
            list_a = [list_a]
        if not isinstance(list_b[0], (list, tuple)):
            list_b = [list_b]
        
        # 检查无效区间
        is_a_invalid = (list_a == [invalid_interval]) or (list_a == invalid_interval)
        is_b_invalid = (list_b == [invalid_interval]) or (list_b == invalid_interval)
        
        # 处理特殊无效情况
        if is_a_invalid and is_b_invalid:
            print("Output IoU: 1.0 (Both invalid)")
            return 1.0
        if is_a_invalid or is_b_invalid:
            print("Output IoU: 0.0 (One invalid)")
            return 0.0
        
        # 合并区间（仅当需要时）
        merged_a = merge_intervals(list_a) if len(list_a) > 1 else list_a
        merged_b = merge_intervals(list_b) if len(list_b) > 1 else list_b
        
        # 计算长度
        len_a = sum(max(0, end - start) for start, end in merged_a)
        len_b = sum(max(0, end - start) for start, end in merged_b)
        ...
        if union <= 0:
            print("Output IoU: 1.0 (Zero union)")
            return 1.0
        
        iou = intersection / union
        print(f"Output IoU: {iou:.4f}")
        return iou
    ...
    for content, sol in zip(completions, solution): # Added video_durations
        ...
        if match_glue:
            ...
            if is_valid_single_interval_format(glue):
...
#     "In the <glue> </glue> tags, present the precise time period in seconds of the video clips on which you base your answer to this question in the format of [s1, e1]. For example: <answer>Abnormal</answer><glue>[5.2, 10.4]</glue>. "
    ...
    def create_dataset_from_csv(file_path, split_name):
        ...
        with open(file_path, mode='r', encoding='utf-8') as csv_file:
            ...
            for row in reader:
                ...
                glue = [round(float(row['start time']), 1), round(float(row['end time']), 1)]
                ...
                normal_list = ['Normal', 'frontdoor', 'highway', 'mall', 'office', 'park', 'parkinglot', 'street', 'restaurant', 'road', 'shop', 'sidewalk', 'street_highview', 'train', 'warehouse']
                if any(norm in original_name for norm in normal_list):
                    answer = "Normal"
                else:
                    answer = "Anomaly"
