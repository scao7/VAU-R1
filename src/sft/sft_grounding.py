...
dataset_path_dict = {
        "msad": "msad/MSAD_train",
        "ucf": "ucf-crime/all_videos",
        "ecva": "ecva"
    }
...
def load_csv_dataset(train_data_path, eval_data_path, video_folder, preprocessed_data_path=None): # Modified to accept preprocessed_data_path
    def create_dataset_from_csv(file_path, split_name):
        ...
        with open(file_path, 'r') as f:
            ...
            for row in reader:
                raw_name = row['Video Name']
                prefix = raw_name.split("_")[0]
                assert prefix in ["msad", "ucf", "ecva"], f"Unknown prefix {prefix} in {file_path}"

                video_name = "_".join(raw_name.split("_")[1:])
                video_path = os.path.join(video_folder, dataset_path_dict[prefix], video_name)
                ...
                example = {
                    ...
                    "solution": {
                        "answer": "Normal" if float(row['start time']) == -1 else "Anomaly",
                        "glue":(float(row['start time']), float(row['end time'])),
                    },
...
# CUDA_VISIBLE_DEVICES=0,1,2 sh training_scripts/run_sft_video_tag.sh 
